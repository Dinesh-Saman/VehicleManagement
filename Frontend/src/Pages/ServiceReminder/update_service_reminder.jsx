...
const UpdateServiceReminder = () => {
  ...
  // Service types (same as in AddServiceReminder)
  const serviceTypes = [
  ...
  // Fetch vehicles and reminder details on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch specific reminder details
        const reminderResponse = await axios.get(`http://localhost:3001/service-reminder/get-reminder/${id}`);
        const reminder = reminderResponse.data;

        // Populate form with existing reminder data
        setSelectedVehicle(reminder.vehicle._id);
        setServiceType(reminder.serviceType);
        setDueDate(reminder.dueDate ? new Date(reminder.dueDate).toISOString().split('T')[0] : '');
        setDueMileage(reminder.dueMileage || '');
        setPriority(reminder.priority || 'Medium');
        setNotes(reminder.notes || '');
        setEstimatedCost(reminder.estimatedCost || '');
        setServiceProvider(reminder.serviceProvider || '');
        setRecurringInterval(reminder.recurringInterval || '');
        setStatus(reminder.status || 'Pending');

        setIsLoading(false);
      } catch (error) {
        console.error('Error fetching data:', error);
        swal("Error", "Failed to load reminder details", "error");
        setIsLoading(false);
      }
    };
  ...
  // Input change handlers (similar to AddServiceReminder)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`http://localhost:3001/service-reminder/update-reminder/${id}`, updatedReminder);
      swal("Success", "Service reminder updated successfully!", "success")
        .then(() => navigate('/view-service-reminder')); // Redirect after successful update
    } catch (error) {
      console.error(error);
      swal("Error", "Failed to update service reminder. Please try again.", "error");
    }
  };
          ...
          <Box display="flex" width="100%">
            ...
            >
              ...
              <Typography variant="h6" style={{ marginTop: 20, color: '#555' }}>
                Update Your Service Reminder Details
              </Typography>
...
export default UpdateServiceReminder;